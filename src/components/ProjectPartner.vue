    ...
    <div class="PP_line1">
      ...
      <div>
        <label class="PP_base PP_select">
          <span>院系</span>
          <select name="from" :disabled="isdisabled">
...
export default {
  name: "ProjectPartner",
  props: ["position", "deleteable", "id", "isdisabled"],
  data() {
    return {
      vc_show: true,
    };
  ...
  computed: {
    DelBtn_isShow() {
      if (this.position === "成员") return true;
      else return false;
    },
  },
  methods: {
    del_parter() {
      // debugger;
      if (this.deleteable) {
        console.log(this.deleteable);
        this.$bus.$emit("delmember", this.id);
      }
    },
  },
};
